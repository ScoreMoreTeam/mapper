...
def get_teams(fbref: sd.FBref) -> List[str]:
    seasons = fbref.read_team_season_stats()
    # Utwórz folder, jeśli nie istnieje
    Path("data/csv").mkdir(parents=True, exist_ok=True)

    # Zapisz do pliku CSV (bez indeksu, żeby nie było podwójnego)
    seasons.to_csv("data/csv/fbref_seasons.csv", index=False)
    seasons_reset = seasons.reset_index()
    seasons_reset.to_csv("data/csv/fbref_seasons_reset.csv", index=False)

    # Przykład DataFrame
    teams = seasons_reset[['team', 'url']].drop_duplicates()
    ...
    Path("data/json/fbref").mkdir(parents=True, exist_ok=True)

    teams.columns = [col if isinstance(col, str) else col[0] for col in teams.columns]
    teams.to_json('data/json/fbref/teams_fbref.json', orient='records', force_ascii=False, indent=4)
...
def get_fbref_players(fbref: sd.FBref) -> list[str]:
    players_df = fbref.read_player_season_stats(stat_type="standard")
    players_df.to_csv("data/csv/fbref_players.csv")
    players_list = players_df.index.get_level_values("player").unique().tolist()

    with open("data/json/fbref/players_fbref.json", "w", encoding="utf-8") as f:
